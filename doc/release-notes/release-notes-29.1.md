BitcoinII Core version 29.1 is now available from:
...
- The minimum block feerate (`-blockmintxfee`) has been changed to 1 satooshi per kvB. It can still be changed using the
configuration option.

- The default minimum relay feerate (`-minrelaytxfee`) and incremental relay feerate (`-incrementalrelayfee`) have been
changed to 100 satooshis per kvB. They can still be changed using their respective configuration options, but it is
...
- #32719 doc, windows: CompanyName "BitcoinII" => "BitcoinII Core project"
