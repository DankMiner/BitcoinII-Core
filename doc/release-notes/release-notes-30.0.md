...
BitcoinII Core version v30.0 is now available from:
...
- The minimum block feerate (`-blockmintxfee`) has been changed to 0.001 satooshi per
...
- The default minimum relay feerate (`-minrelaytxfee`) and incremental relay feerate
  (`-incrementalrelayfee`) have been changed to 0.1 satooshis per vB. They can still
...
- The new `bitcoinII` command does support one new feature: an (experimental) IPC Mining
...
- The `test_bitcoinII` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `bitcoinII` command
  as `bitcoinII test`. The `libexec/` directory also contains new `bitcoinII-node` and
  `bitcoinII-gui` binaries which support IPC features and are called through the
  `bitcoinII` tool. In source builds only, `test_bitcoinII-qt`, `bench_bitcoinII`, and
  `bitcoinII-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `bitcoinII` command. See `bitcoinII help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of BitcoinII Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in BitcoinII Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the bitcoinII-wallet tool,
