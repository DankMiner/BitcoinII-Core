...
```bash
./build/bin/bitcoinII-d -signet -daemon
```
...
for ((n=1;n<=3;n++))
do
 ./build/bin/bitcoinII rpc -signet createwallet "participant_${n}"
done
```
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/bitcoinII rpc -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))' | sed 's /0/\* /<0;1>/* ')
done
```
...
checksum=$(./build/bin/bitcoinII rpc -signet getdescriptorinfo $desc | jq -r '.checksum')
...
Documentation for these and other parameters can be found by typing `./build/bin/bitcoinII rpc -signet help importdescriptors`.
...
```bash
./build/bin/bitcoinII rpc -signet createwallet "multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/bitcoinII rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/bitcoinII rpc -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/bitcoinII rpc -signet loadwallet "participant_${n}"; done
./build/bin/bitcoinII rpc -signet loadwallet "multisig_wallet_01"
```
...
```bash
receiving_address=$(./build/bin/bitcoinII rpc -signet -rpcwallet="multisig_wallet_01" getnewaddress)
...
```bash
./build/bin/bitcoinII rpc -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./build/bin/bitcoinII rpc -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/bitcoinII rpc -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/bitcoinII rpc -signet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/bitcoinII rpc -signet decodepsbt $funded_psbt

./build/bin/bitcoinII rpc -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/bitcoinII rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/bitcoinII rpc -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/bitcoinII rpc -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/bitcoinII rpc -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/bitcoinII rpc -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/bitcoinII rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/bitcoinII rpc -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/bitcoinII rpc -signet finalizepsbt $psbt_2 | jq -r '.hex')
