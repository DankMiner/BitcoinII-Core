...
`./`               | `bitcoinII.conf`        | User-defined [configuration settings](bitcoinII-conf.md) for `bitcoinII-d` or `bitcoinII-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `bitcoinII-d.pid`        | Stores the process ID (PID) of `bitcoinII-d` or `bitcoinII-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bitcoinII-d` or `bitcoinII-qt`; can be specified by `-debuglogfile` option
...
This table describes the files installed by BitcoinII Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| bitcoinII.conf                                               | [Generated](../contrib/devtools/gen-bitcoinII-conf.sh) configuration file     |
| bin/bitcoinII                                                | Command-line tool for interacting with BitcoinII. Calls other binaries below. |
| bin/bitcoinII-cli                                            | Tool for making node and wallet RPC calls.                                  |
| bin/bitcoinII-qt                                             | BitcoinII node and wallet GUI                                                 |
| bin/bitcoinII-tx                                             | Tool for creating and modifying transactions                                |
| bin/bitcoinII-util                                           | Miscellaneous utilities                                                     |
| bin/bitcoinII-wallet                                         | BitcoinII wallet tool                                                         |
| bin/bitcoinII-d                                               | BitcoinII node and wallet daemon                                              |
| *lib/libbitcoinIIkernel.so*                                  | Shared library containing core consensus and validation code                |
| *lib/pkgconfig/libbitcoinIIkernel.pc*                        | Pkg-config metadata for linking to `libbitcoinIIkernel`                       |
| *libexec/bench_bitcoinII*                                    | Benchmarking tool for measuring node performance                            |
| *libexec/bitcoinII-chainstate*                               | Tool to validate and connect blocks                                         |
| libexec/bitcoinII-gui                                        | IPC-enabled alternative to `bitcoinII-qt`                                     |
| libexec/bitcoinII-node                                       | IPC-enabled alternative to `bitcoinII-d`                                       |
| libexec/test_bitcoinII                                       | Unit test binary                                                            |
| *libexec/test_bitcoinII-qt*                                  | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `bitcoinII-cli`, `bitcoinII-d`, and others |
...
- README and bitcoinII.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `bitcoinII-cli.exe`).
