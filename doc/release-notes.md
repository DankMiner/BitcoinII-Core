...
BitcoinII Core version 31.1 is now available from:
...
(on Windows) or just copy over `/Applications/BitcoinII-Qt` (on macOS) or
...
migrated. Old wallet versions of BitcoinII Core are generally supported.

Compatibility
==============

BitcoinII Core is supported and tested on the following operating systems or
newer: Linux Kernel 3.17, macOS 14, and Windows 10 (version 1903). BitcoinII Core
should also work on most other Unix-like systems but is not as frequently tested
on them. It is not recommended to use BitcoinII Core on unsupported systems.
...
- #61(bitcoinII-core/leveldb): Disable seek compaction
...
- #35080 test: Add missing self.options.timeout_factor scale in tool_bitcoinII_chainstate.py
