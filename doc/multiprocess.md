...
The `-DENABLE_IPC=ON` build option, supported and enabled by default on Unix systems, can be passed to build the supplemental `bitcoinII-node` and `bitcoinII-gui` multiprocess executables.
...
cmake --build build
build/bin/bitcoinII -m node -regtest -printtoconsole -debug=ipc
BITCOINII_CMD="bitcoinII -m" build/test/functional/test_runner.py
```
