...
class WalletTest(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        # node0 should end up with 100 bc2 in block rewards plus fees, but
        ...
        fee_rate_sat2_vb = fee_per_byte * Decimal(1e8)
        txid = self.nodes[2].sendtoaddress(address, 10, "", "", False, fee_rate=fee_rate_sat2_vb)
        ...
        txid = self.nodes[2].sendtoaddress(address, 10, "", "", True, fee_rate=fee_rate_sat2_vb)
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        ...
        # Sendmany 10 BC2
        txid = self.nodes[2].sendmany('', {address: 10}, 0, "", [], fee_rate=fee_rate_sat2_vb)
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        ...
        txid = self.nodes[2].sendmany('', {address: 10}, 0, "", [address], fee_rate=fee_rate_sat2_vb)
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node_2_bal -= Decimal('10')
        assert_equal(self.nodes[2].getbalance(), node_2_bal)
        ...
        txid = self.nodes[2].sendmany(dummy='', amounts={a0: 5, a1: 5}, subtractfeefrom=[a0, a1], fee_rate=fee_rate_sat2_vb)
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node_2_bal -= Decimal('10')
        assert_equal(self.nodes[2].getbalance(), node_2_bal)
        ...
        self.log.info("Test sendmany with fee_rate param (explicit fee rate in sat2/vB)")
        fee_rate_sat2_vb = 2
        fee_rate_bc2_kvb = fee_rate_sat2_vb * 1e3 / 1e8
        explicit_fee_rate_bc2_kvb = Decimal(fee_rate_bc2_kvb) / 1000

        # Test passing fee_rate as a string
        txid = self.nodes[2].sendmany(amounts={address: 10}, fee_rate=str(fee_rate_sat2_vb))
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_bc2_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        txid = self.nodes[2].sendmany(amounts={address: amount}, fee_rate=fee_rate_sat2_vb)
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_bc2_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        assert_raises_rpc_error(-6, "Fee rate (0.999 sat2/vB) is lower than the minimum fee rate setting (1.000 sat2/vB)",
        ...
        msg = "Fee rate (0.000 sat2/vB) is lower than the minimum fee rate setting (1.000 sat2/vB)"
        ...
        for target, mode in product([-1, 0], ["bc2/kb", "sat2/b"]):
