...
class FeatureFastpruneTest(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        # BitcoinII forbids Taproot annex data. Build a large transaction
        # using ordinary spendable outputs instead.
        tx = wallet.create_self_transfer_multi(num_outputs=2000)["tx"]
        assert len(tx.serialize()) > 0x10000
