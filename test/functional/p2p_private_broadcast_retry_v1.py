...
* Get one successful connection to a Tor peer (.onion) so that bitcoinII-d assumes the configured
...
class P2PPrivateBroadcastRetryV1(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        # The destinations behind the -proxy= don't actually support v2. When bitcoinII-d runs with -v2transport=1
        ...
        self.log.info("Opening a connection to a Tor addresses, so bitcoinII-d considers -onion= is a real Tor proxy")
