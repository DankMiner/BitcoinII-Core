...
DEFAULT_BLOCK_MIN_TX_FEE = 1 # default `-blockmintxfee` setting [sat2/kvB]
...
class MiningTest(BitcoinIITestFramework):
    ...
    def test_fees_and_sigops(self):
        ...
        # higher fees (100 sat2/vbyte) and descends from a different coinbase.
    ...
    def test_blockmintxfee_parameter(self):
        ...
        # test default (no parameter), zero and a bunch of arbitrary blockmintxfee rates [sat2/kvB]
        for blockmintxfee_sat2_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 5, 10, 50, 100, 500, 1000, 2500, 5000, 21000, 333333, 2500000):
            blockmintxfee_bc2_kvb = blockmintxfee_sat2_kvb / Decimal(COIN)
            if blockmintxfee_sat2_kvb == DEFAULT_BLOCK_MIN_TX_FEE:
                self.log.info(f"-> Default -blockmintxfee setting ({blockmintxfee_sat2_kvb} sat2/kvB)...")
            else:
                blockmintxfee_parameter = f"-blockmintxfee={blockmintxfee_bc2_kvb:.8f}"
                self.log.info(f"-> Test {blockmintxfee_parameter} ({blockmintxfee_sat2_kvb} sat2/kvB)...")
                self.restart_node(0, extra_args=[blockmintxfee_parameter, '-minrelaytxfee=0', '-persistmempool=0'])
            assert_equal(node.getmininginfo()['blockmintxfee'], blockmintxfee_bc2_kvb)

            # submit one tx with exactly the blockmintxfee rate, and one slightly below
            tx_with_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_bc2_kvb, confirmed_only=True)
            assert_equal(tx_with_min_feerate["fee"], get_fee(tx_with_min_feerate["tx"].get_vsize(), blockmintxfee_bc2_kvb))
            if blockmintxfee_sat2_kvb >= 10:
                lowerfee_bc2_kvb = blockmintxfee_bc2_kvb - Decimal(10)/COIN  # 0.01 sat2/vbyte lower
                assert_greater_than(blockmintxfee_bc2_kvb, lowerfee_bc2_kvb)
                assert_greater_than_or_equal(lowerfee_bc2_kvb, 0)
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=lowerfee_bc2_kvb, confirmed_only=True)
                assert_equal(tx_below_min_feerate["fee"], get_fee(tx_below_min_feerate["tx"].get_vsize(), lowerfee_bc2_kvb))
            else:  # go below zero fee by using modified fees
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_bc2_kvb, confirmed_only=True)
