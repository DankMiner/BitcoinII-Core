...
class TaprootTest(BitcoinIITestFramework):
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        while left:
            ...
            fee = min(random.randrange(MIN_FEE * 2, MIN_FEE * 4), amount - DUST_LIMIT)  # 10000-20000 sat2 fee
            in_value = amount - fee
    ...
    def gen_test_vectors(self):
        ...
        for i in range(len(input_spks)):
            utxos_spent.append({"scriptPubKey": inputs[i].scriptPubKey.hex(), "amountSat2s": inputs[i].nValue})
