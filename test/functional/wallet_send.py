...
class WalletSendTest(BitcoinIITestFramework):
    def set_test_params(self):
        ...
        getcontext().prec = 8 # Satooshi precision for Decimal
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-8, "Use fee_rate (sat2/vB) instead of feeRate", w0.send, {w1.getnewaddress(): 1}, 6, "conservative", 1, {"feeRate": 0.01})
        ...
        # Test setting explicit fee rate just below the minimum of 1 sat2/vB.
        self.log.info("Explicit fee rate raises RPC error 'fee rate too low' if fee_rate of 0.99999999 is passed")
        msg = "Fee rate (0.999 sat2/vB) is lower than the minimum fee rate setting (1.000 sat2/vB)"
        ...
        # res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0.1, estimate_mode="sat2/b", add_to_wallet=False)
        # assert res["hex"]
        # hex = res["hex"]
        ...
        res = self.test_send(
            ...
            add_inputs=True,
            psbt=True,
            fee_rate=target_fee_rate_sat2_vb
        )
        signed = ext_wallet.walletprocesspsbt(res["psbt"])
        signed = ext_fund.walletprocesspsbt(res["psbt"])
        assert signed["complete"]
        ...
        actual_fee_rate_sat2_vb = Decimal(testres["fees"]["base"]) * Decimal(1e8) / Decimal(testres["vsize"])
        ...
        assert_equal(round(actual_fee_rate_sat2_vb), target_fee_rate_sat2_vb)
