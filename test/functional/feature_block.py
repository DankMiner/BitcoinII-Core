...
class FullBlockTest(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        # Extend the b26 chain to make sure bitcoinII-d isn't accepting b26
        ...
        # Extend the b28 chain to make sure bitcoinII-d isn't accepting b28
        ...
        # Create a transaction that spends one satooshi to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 satooshi to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        # bitcoinII-d doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #                        \-> b68 (20)
        #
        # b68 - coinbase with an extra 10 satooshis,
        #       creates a tx that has 9 satooshis from out[20] go to fees
        #       this fails because the coinbase is trying to claim 1 satooshi too much in fees
        #
        # b69 - coinbase with extra 10 satooshis, and a tx that gives a 10 satooshi fee
        #       this succeeds
        #
        ...
        tx4 = self.create_tx(tx1, 3, 0, CScript([OP_RETURN]))
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, *, script=None, version=4, additional_output_scripts=None):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one satooshi to fees
            tx = self.create_tx(spend, 0, 1, output_script=script)  # spend 1 satooshi
            self.sign_tx(tx, spend)
