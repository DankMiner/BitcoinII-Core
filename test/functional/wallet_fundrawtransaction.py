...
class RawTransactionsTest(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        self.fee_rate_sat2s_per_vb = self.min_relay_tx_fee * Decimal(1e8) / 1000
    ...
    def test_duplicate_outputs(self):
        ...
        self.nodes[0].sendtoaddress(addr, 5, fee_rate=self.fee_rate_sat2s_per_vb)
        ...
        res = w.fundrawtransaction(tx_hex, add_inputs=True, fee_rate=self.fee_rate_sat2s_per_vb)
        ...
        res_sffo = w.fundrawtransaction(tx_hex, add_inputs=True, subtractFeeFromOutputs=[0,1], fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_change_position(self):
        ...
        rawmatch = self.nodes[2].fundrawtransaction(rawmatch, changePosition=1, subtractFeeFromOutputs=[0], fee_rate=self.fee_rate_sat2s_per_vb)
        ...
        self.nodes[0].sendtoaddress(self.nodes[3].get_wallet_rpc(self.default_wallet_name).getnewaddress(), self.watchonly_amount / 10, fee_rate=self.fee_rate_sat2s_per_vb)

        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), 1.5, fee_rate=self.fee_rate_sat2s_per_vb)
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), 1.0, fee_rate=self.fee_rate_sat2s_per_vb)
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), 5.0, fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_simple(self):
        ...
        rawtxfund = self.nodes[2].fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_valid_change_address(self):
        ...
        rawtxfund = self.nodes[2].fundrawtransaction(rawtx, changeAddress=change, changePosition=0, fee_rate=self.fee_rate_sat2s_per_vb)
        dec_tx  = self.nodes[2].decoderawtransaction(rawtxfund['hex'])
        out = dec_tx['vout'][0]
    ...
    def test_change_type(self):
        ...
        rawtx = self.nodes[2].fundrawtransaction(rawtx, change_type='bech32', fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_two_vin(self):
        ...
        # Should fail without add_inputs:
        assert_raises_rpc_error(-4, ERR_NOT_ENOUGH_PRESET_INPUTS, self.nodes[2].fundrawtransaction, rawtx, add_inputs=False)
        rawtxfund = self.nodes[2].fundrawtransaction(rawtx, add_inputs=True, fee_rate=self.fee_rate_sat2s_per_vb)
        dec_tx  = self.nodes[2].decoderawtransaction(rawtxfund['hex'])
        matchingOuts = 0
        for out in dec_tx['vout']:
            if out['scriptPubKey']['address'] in outputs:
    ...
    def test_fee_p2pkh(self):
        ...
        fundedTx = self.nodes[0].fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb)

        # Create same transaction over sendtoaddress.
        txId = self.nodes[0].sendtoaddress(self.nodes[1].getnewaddress(), 1.1, fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_fee_p2pkh_multi_out(self):
        ...
        # Create same transaction over sendtoaddress.
        txId = self.nodes[0].sendmany("", outputs, fee_rate=self.fee_rate_sat2s_per_vb)
        signedFee = self.nodes[0].getmempoolentry(txId)['fees']['base']
    ...
    def test_fee_p2sh(self):
        ...
        # Create same transaction over sendtoaddress.
        txId = self.nodes[0].sendtoaddress(mSigObj, 1.1, fee_rate=self.fee_rate_sat2s_per_vb)
        signedFee = self.nodes[0].getmempoolentry(txId)['fees']['base']
    ...
    def test_spend_2of2(self):
        ...
        self.nodes[0].sendtoaddress(mSigObj["address"], 1.2, fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[0], 1)
    ...
    def test_locked_wallet(self):
        ...
        funded_tx = wallet.fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb)
        ...
        fundedTx = wallet.fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb)
        ...
        with WalletUnlock(wallet, "test"):
            ...
            wallet.sendall(recipients=[df_wallet.getnewaddress(), df_wallet.getnewaddress(), df_wallet.getnewaddress()], fee_rate=self.fee_rate_sat2s_per_vb)
        wallet.unloadwallet()
        self.generate(self.nodes[1], 1)
    ...
    def test_many_inputs_fee(self):
        ...
        self.nodes[1].sendall(recipients=[self.nodes[0].getnewaddress()], fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[1], 1)

        for _ in range(20):
            self.nodes[0].sendtoaddress(self.nodes[1].getnewaddress(), 0.01, fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[0], 1)
        ...
        fundedTx = self.nodes[1].fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb)

        # Create same transaction over sendtoaddress.
        txId = self.nodes[1].sendmany("", outputs, fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat2/vB and feeRate BC2/kvB)")
        node = self.nodes[3]
        ...
        result = node.fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb)  # uses self.min_relay_tx_fee (set by fee_rate in sat2/vB)
        bc2_kvb_to_sat2_vb = 100000  # (1e5)
        result1 = node.fundrawtransaction(rawtx, fee_rate=str(2 * bc2_kvb_to_sat2_vb * self.min_relay_tx_fee))
        result2 = node.fundrawtransaction(rawtx, feeRate=2 * self.min_relay_tx_fee)
        result3 = node.fundrawtransaction(rawtx, fee_rate=10 * bc2_kvb_to_sat2_vb * self.min_relay_tx_fee)
        ...
        # With no arguments passed, expect fee of 141 satooshis.
        assert_approx(node.fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb)["fee"], vexp=0.00000141, vspan=0.00000001)
        ...
        # Test fee_rate values that cannot be represented in sat2/vB.
        for invalid_value in [0.0001, 0.00000001, 0.00099999, 31.99999999]:
            assert_raises_rpc_error(-3, "Invalid amount",
                node.fundrawtransaction, rawtx, fee_rate=invalid_value, add_inputs=True)

        self.log.info("Test min fee rate checks are bypassed with fundrawtxn, e.g. a fee_rate under 1 sat2/vB is allowed")
        node.fundrawtransaction(rawtx, fee_rate=0.999, add_inputs=True)
        node.fundrawtransaction(rawtx, feeRate=0.00000999, add_inputs=True)

        self.log.info("- raises RPC error if both feeRate and fee_rate are passed")
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat2/vB) and feeRate (BC2/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        result = [self.nodes[3].fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb),
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[], fee_rate=self.fee_rate_sat2s_per_vb),  # empty subtraction list
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0], fee_rate=self.fee_rate_sat2s_per_vb), # uses self.min_relay_tx_fee (set by fee_rate in sat2/vB)
        ...
        # Test subtract fee from outputs with fee_rate (sat2/vB)
        bc2_kvb_to_sat2_vb = 100000  # (1e5)
        result = [self.nodes[3].fundrawtransaction(rawtx, fee_rate=self.fee_rate_sat2s_per_vb),  # uses self.min_relay_tx_fee (set by fee_rate in sat2/vB)
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[], fee_rate=self.fee_rate_sat2s_per_vb),  # empty subtraction list
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0], fee_rate=self.fee_rate_sat2s_per_vb),  # uses self.min_relay_tx_fee (set by fee_rate in sat2/vB)
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * bc2_kvb_to_sat2_vb * self.min_relay_tx_fee),
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * bc2_kvb_to_sat2_vb * self.min_relay_tx_fee, subtractFeeFromOutputs=[0]),]
        dec_tx = [self.nodes[3].decoderawtransaction(tx_['hex']) for tx_ in result]
        output = [d['vout'][1 - r['changepos']]['value'] for d, r in zip(dec_tx, result)]
        change = [d['vout'][r['changepos']]['value'] for d, r in zip(dec_tx, result)]
        ...
        # satooshis more, than outputs 2 and 3.
    ...
    def test_subtract_fee_with_presets(self):
        ...
        fundedtx = self.nodes[0].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0], fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_transaction_too_large(self):
        ...
        wallet.sendmany("", outputs, fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_external_inputs(self):
        ...
        self.nodes[0].sendtoaddress(addr, 10, fee_rate=self.fee_rate_sat2s_per_vb)
        self.nodes[0].sendtoaddress(wallet.getnewaddress(), 10, fee_rate=self.fee_rate_sat2s_per_vb)
        ...
        funded_tx = wallet.fundrawtransaction(raw_tx, solving_data={"descriptors": [desc]}, fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_add_inputs_default_value(self):
        ...
        self.nodes[0].sendtoaddress(addr1, 5, fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[0], 1)
        ...
        tx = wallet.send(outputs=[{addr1: 3}], fee_rate=self.fee_rate_sat2s_per_vb)
        assert tx["complete"]
        ...
        source_tx = self.nodes[0].send(outputs=[{addr1: 5}, {addr2: 5}], change_position=0, fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[0], 1)
        ...
        options["fee_rate"] = self.fee_rate_sat2s_per_vb
        ...
        assert "psbt" in wallet.walletcreatefundedpsbt(inputs=inputs, outputs=outputs, options={'fee_rate': self.fee_rate_sat2s_per_vb})
        ...
        outputs = {self.nodes[1].getnewaddress(): 8}
        assert_raises_rpc_error(-4, ERR_NOT_ENOUGH_PRESET_INPUTS, wallet.walletcreatefundedpsbt, inputs=inputs, outputs=outputs, options={'fee_rate': self.fee_rate_sat2s_per_vb})
    ...
    def test_preset_inputs_selection(self):
        ...
        self.nodes[0].sendmany("", outputs, fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[0], 1)
        ...
        options = {
            ...
            "add_to_wallet": False,
            "fee_rate": self.fee_rate_sat2s_per_vb
        }
    ...
    def test_weight_calculation(self):
        ...
        self.nodes[0].sendtoaddress(wallet.getnewaddress(address_type="bech32"), 5, fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[0], 1)
    ...
    def test_weight_limits(self):
        ...
        txid = self.nodes[0].send(outputs=outputs, change_position=0, fee_rate=self.fee_rate_sat2s_per_vb)["txid"]
        self.generate(self.nodes[0], 1)
    ...
    def test_include_unsafe(self):
        ...
        fundedtx = wallet.fundrawtransaction(rawtx, include_unsafe=True, fee_rate=self.fee_rate_sat2s_per_vb)
        ...
        self.generate(self.nodes[0], 1)
        fundedtx = wallet.fundrawtransaction(rawtx, include_unsafe=False, fee_rate=self.fee_rate_sat2s_per_vb)
        tx_dec = wallet.decoderawtransaction(fundedtx['hex'])
        assert all((txin["txid"], txin["vout"]) in inputs for txin in tx_dec["vin"])
        signedtx = wallet.signrawtransactionwithwallet(fundedtx['hex'])
        assert wallet.testmempoolaccept([signedtx['hex']])[0]["allowed"]
    ...
    def test_feerate_rounding(self):
        ...
        addr = w.getnewaddress(address_type="bech32")
        self.nodes[0].sendtoaddress(addr, 1, fee_rate=self.fee_rate_sat2s_per_vb)
        self.generate(self.nodes[0], 1)

        # A P2WPKH input costs 68 vbytes; With a single P2WPKH output, the rest of the tx is 42 vbytes for a total of 110 vbytes.
        # At a feerate of 1.85 sat2/vb, the input will need a fee of 125.8 sat2s and the rest 77.7 sat2s
        # The entire tx fee should be 203.5 sat2s.
    ...
    def test_input_confs_control(self):
        ...
        for _ in range(2):
            self.nodes[2].sendmany("", {wallet.getnewaddress():1, wallet.getnewaddress():1}, fee_rate=self.fee_rate_sat2s_per_vb)
            self.generate(self.nodes[2], 1)

        unconfirmed_txid = wallet.sendtoaddress(wallet.getnewaddress(), 0.5, fee_rate=self.fee_rate_sat2s_per_vb)
    ...
    def test_cannot_cover_fees(self):
        ...
        # Set up wallet with 2 utxos: 0.3 BC2 and 0.15 BC2
        ...
        self.log.info("Attempt to send 0.45 BC2 without SFFO")
        ...
        self.log.info("Send 0.45 BC2 with SFFO")
        wallet.fundrawtransaction(rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.45 BC2 by restricting coin selection with minconf=6")
        ...
        self.log.info("Attempt to send 0.45 BC2 preselecting 0.15 BC2 utxo")
        rawtx = wallet.createrawtransaction(inputs=[{"txid": txid2, "vout": vout2}], outputs=[{default_wallet.getnewaddress(): 0.45}])
        assert_raises_rpc_error(-4, amount_with_fee_err_msg.format("0.00000042"), wallet.fundrawtransaction, rawtx, options={"fee_rate":1})

        self.log.info("Send 0.45 BC2 preselecting 0.15 BC2 utxo with SFFO")
        wallet.fundrawtransaction(hexstring=rawtx, options={"subtractFeeFromOutputs":[0]})

        self.log.info("Attempt to send 0.15 BC2 using only the 0.15 BC2 preselected utxo")
        ...
        self.log.info("Send 0.15 BC2 using only the 0.15 BC2 preselected utxo with SFFO")
