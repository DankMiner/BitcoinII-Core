...
class MempoolAcceptanceTest(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        # Settings are listed in BC2/kvB
        ...
        self.check_mempool_result(
            result_expected=[{'txid': tx.txid_hex, 'allowed': False, 'reject-reason': 'bitcoinII-bare-multisig'}],
            rawtxs=[tx.serialize().hex()],
        )
        tx = tx_from_hex(raw_tx_reference)
        ...
        # BitcoinII permits at most one OP_RETURN output per
        # transaction and caps its complete scriptPubKey at 83 bytes.
        self.log.info("Multiple OP_RETURN outputs are rejected")
        tx = tx_from_hex(raw_tx_reference)
        tx.vout.append(
            CTxOut(0, CScript([OP_RETURN, b'\xff']))
        )
        tx.vout.append(
            CTxOut(0, CScript([OP_RETURN, b'\xff' * 50000]))
        )
        self.check_mempool_result(
            result_expected=[{
                'txid': tx.txid_hex,
                'allowed': False,
                'reject-reason': 'bitcoinII-opreturn-count',
            }],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )

        self.log.info("Many OP_RETURN outputs are rejected")
        tx = tx_from_hex(raw_tx_reference)
        op_return_count = 42
        tx.vout[0].nValue = int(
            tx.vout[0].nValue / op_return_count
        )
        ...
        self.log.info(
            "Oversized single OP_RETURN output is rejected"
        )
        tx = tx_from_hex(raw_tx_reference)
        tx.vout[0].scriptPubKey = CScript([OP_RETURN])

        data_len = (
            int(MAX_STANDARD_TX_WEIGHT / 4)
            - tx.get_vsize()
            - 5
            - 4
        )
        ...
        self.check_mempool_result(
            result_expected=[{
                'txid': tx.txid_hex,
                'allowed': False,
                'reject-reason': 'bitcoinII-opreturn-size',
            }],
            rawtxs=[tx.serialize().hex()],
        )

        # The BitcoinII OP_RETURN limit is encountered before the
        # generic standard-transaction-size limit here as well.
        tx.vout[0].scriptPubKey = CScript([
            OP_RETURN,
            b'\xff' * (data_len + 1),
        ])
        ...
        self.log.info('Creating a new bare multisig output is rejected by BitcoinII')
        ...
        # BitcoinII mempool policy rejects creation of new bare
        # multisig outputs.
        self.check_mempool_result(
            result_expected=[{
                'txid': tx.txid_hex,
                'allowed': False,
                'reject-reason': 'bitcoinII-bare-multisig',
            }],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )

        # The same restriction is consensus-enforced, so it cannot
        # be bypassed by including the transaction directly in a block.
        assert_raises_rpc_error(
            -25,
