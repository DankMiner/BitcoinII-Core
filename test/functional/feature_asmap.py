...
Verify node behaviour and debug log when launching bitcoinII-d with different
...
class AsmapTest(BitcoinIITestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test bitcoinII-d with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_noasmap_arg(self):
        self.log.info('Test bitcoinII-d with -noasmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0, ["-noasmap"])

    def test_asmap_with_absolute_path(self):
        self.log.info('Test bitcoinII-d -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test bitcoinII-d -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_embedded_asmap(self):
        if self.is_embedded_asmap_compiled():
            self.log.info('Test bitcoinII-d -asmap (using embedded map data)')
            for arg in ['-asmap', '-asmap=1']:
                self.stop_node(0)
                with self.node.assert_debug_log(["Opened asmap data", "from embedded byte array"]):
                    self.start_node(0, [arg])
        else:
            self.log.info('Test bitcoinII-d -asmap (compiled without embedded map data)')
            for arg in ['-asmap', '-asmap=1']:
                self.stop_node(0)
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info("Test bitcoinII-d -asmap restart with addrman containing new and tried entries")
        self.stop_node(0)
    ...
    def test_asmap_with_missing_file(self):
        self.log.info('Test bitcoinII-d -asmap with missing map file')
        self.stop_node(0)
    ...
    def test_empty_asmap(self):
        self.log.info('Test bitcoinII-d -asmap with empty map file')
        self.stop_node(0)
    ...
    def test_asmap_health_check(self):
        self.log.info('Test bitcoinII-d -asmap logs ASMap Health Check with basic stats')
