...
"""Check that it's not possible to start a second bitcoinII-d instance using the same datadir or wallet."""
import random
import string
...
class FilelockTest(BitcoinIITestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second bitcoinII-d instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second bitcoinII-d instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second bitcoinII-d using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second bitcoinII-d instance shutdown
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second bitcoinII-d instance using the same wallet")
