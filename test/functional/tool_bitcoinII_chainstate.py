...
"""Test bitcoinII-chainstate tool functionality

Test basic block processing via bitcoinII-chainstate tool, including detecting
duplicates and malformed input.

Test that bitcoinII-chainstate can load a datadir initialized with an assumeutxo
...
class BitcoinIIChainstateTest(BitcoinIITestFramework):
    ...
    def basic_test(self):
        ...
        self.log.info(f"Test bitcoinII-chainstate {self.get_binaries().chainstate_argv()} with datadir: {datadir}")
    ...
    def assumeutxo_test(self, dump_output_path):
        ...
        datadir = n1.chain_path
        n1.stop_node()
        self.log.info(f"Test bitcoinII-chainstate {self.get_binaries().chainstate_argv()} with an assumeutxo datadir: {datadir}")
...
if __name__ == "__main__":
    BitcoinIIChainstateTest(__file__).main()
