...
without the parent process' BitcoinIITestFramework also failing.
"""
...
class FeatureFrameworkStartupFailures(BitcoinIITestFramework):
    ...
    def _verify_startup_failure(self, test, internal_args, expected_exception):
        ...
        try:
            ...
            sys.exit("Unexpected child process timeout!\n"
                     ...
                     "meaning dangling bitcoinII-d processes are to be expected.\n" +
    ...
    def run_test(self):
        ...
        self.log.info("Verifying inability to connect to bitcoinII-d's RPC interface due to wrong port results in one exception containing at least one OSError.")
        self._verify_startup_failure(
            TestWrongRpcPortStartupFailure, [f"--internal_node_start_duration={node_start_duration}"],
            r"AssertionError: \[node 0\] Unable to connect to bitcoinII-d after \d+s \(ignored errors: {[^}]*'OSError \w+'?: \d+[^}]*}, latest: '[\w ]+'/\w+\([^)]+\)\)"
        )
        ...
        self._verify_startup_failure(
            ...
            r"AssertionError: \[node 0\] bitcoinII-d should have exited within \d+s with an error \(cmd:"
        )

        self.log.info("Verifying startup failure due to invalid arg results in only one exception.")
        self._verify_startup_failure(
            TestInitErrorStartupFailure, [],
            r"FailedToStartError: \[node 0\] bitcoinII-d exited with status 1 during initialization\. Error: Error parsing command line arguments: Invalid parameter -nonexistentarg"
        )
...
class InternalDurationTestMixin(InternalTestMixin):
    ...
    def get_reasonable_rpc_timeout(self):
        ...
        # Divide by timeout_factor to counter multiplication in BitcoinIITestFramework.
        return max(3, 2 * self.options.node_start_duration) / self.options.timeout_factor

class TestWrongRpcPortStartupFailure(InternalDurationTestMixin, BitcoinIITestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestMissingInitErrorTimeout(InternalDurationTestMixin, BitcoinIITestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestInitErrorStartupFailure(InternalTestMixin, BitcoinIITestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestStartStopStartupFailure(InternalTestMixin, BitcoinIITestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestSuccess(InternalTestMixin, BitcoinIITestFramework):
    def set_test_params(self):
        self.num_nodes = 1
