...
void ApplyBitcoinIITransactionRules(
...
util::Result<void> ApplyArgsManOptions(const ArgsManager& argsman, const CChainParams& chainparams, MemPoolOptions& mempool_opts)
{
    ...
    ApplyBitcoinIITransactionRules(mempool_opts);
    ...
    LogInfo("BitcoinII transaction rules:");
    LogInfo(
        "* Maximum aggregate OP_RETURN script size: %u bytes",
        MAX_BITCOINII_OP_RETURN_BYTES);
    LogInfo("* OP_RETURN OP_13 opcode: forbidden");
    LogInfo("* Bare multisig outputs: forbidden");
    LogInfo(
        "* Maximum Taproot script-path script size: %u bytes",
        MAX_BITCOINII_TAPSCRIPT_BYTES);
    LogInfo("* Ordinals inscription envelopes: forbidden");
    LogInfo("* Taproot annex: forbidden");
    LogInfo("* Transaction-rule rejection logging: enabled");
