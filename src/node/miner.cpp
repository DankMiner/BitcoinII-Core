...
int64_t UpdateTime(CBlockHeader* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    const bool time_updated = nOldTime < nNewTime;

    if (time_updated) {
        pblock->nTime = nNewTime;
    }

    // Updating candidate time can change required work under ShockWave.
    // Preserve the existing testnet behavior of always recalculating work.
    if (time_updated || consensusParams.fPowAllowMinDifficultyBlocks) {
