...
#include <kernel/bitcoinIIkernel_wrapper.h>
#include <util/fs.h>

#define BOOST_TEST_MODULE BitcoinII Kernel Test Suite
...
{
    Logger logger{std::make_unique<TestLog>()};
    auto test_directory{TestDirectory{"chainman_test_bitcoinII_kernel"}};
...
void chainman_mainnet_validation_test(TestDirectory& test_directory)
{
    ...
    // mainnet block 1
    auto raw_block = hex_string_to_byte_vec("");
    Block block{raw_block};
    BlockHeader header{block.GetHeader()};
    TransactionView tx{block.GetTransaction(block.CountTransactions() - 1)};
    BOOST_CHECK_EQUAL(byte_span_to_hex_string_reversed(tx.Txid().ToBytes()), "e044e483a12293b045d4e10aa510c9ccd97e5b5bfbc9994bcea481d201576d19");
    BOOST_CHECK_EQUAL(header.Version(), 2);
    BOOST_CHECK_EQUAL(header.Timestamp(), 1750577230);
    BOOST_CHECK_EQUAL(header.Bits(), 0x1d00ffff);
    BOOST_CHECK_EQUAL(header.Nonce(), 2786983383);
    BOOST_CHECK_EQUAL(tx.CountInputs(), 1);
    Transaction tx2 = tx;
...
// Disabled until BitcoinII-specific REGTEST_BLOCK_DATA fixtures are available.
BOOST_AUTO_TEST_CASE(btck_block_tree_entry_tests, * boost::unit_test::disabled())
{
    auto test_directory{TestDirectory{"block_tree_entry_test_bitcoinII_kernel"}};
    auto notifications{std::make_shared<TestKernelNotifications>()};
...
// Disabled until BitcoinII-specific REGTEST_BLOCK_DATA fixtures are available.
BOOST_AUTO_TEST_CASE(btck_chainman_in_memory_tests, * boost::unit_test::disabled())
{
    auto in_memory_test_directory{TestDirectory{"in-memory_test_bitcoinII_kernel"}};
...
// Disabled until BitcoinII-specific REGTEST_BLOCK_DATA fixtures are available.
BOOST_AUTO_TEST_CASE(btck_chainman_regtest_tests, * boost::unit_test::disabled())
{
    auto test_directory{TestDirectory{"regtest_test_bitcoinII_kernel"}};
