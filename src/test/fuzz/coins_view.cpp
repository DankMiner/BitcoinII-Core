        ...
        CallOneOf(
            ...
            [&] {
                ...
                if (ContainsSpentInput(transaction, coins_view_cache)) {
                    // Avoid:
                    // Consensus::CheckTxInputs: Assertion `!coin.IsSpent()' failed.
                    return;
                }
                ...
                prev_block.nHeight = fuzzed_data_provider.ConsumeIntegralInRange<int>(-1, std::numeric_limits<int>::max() - 1);
                if (Consensus::CheckTxInputs(transaction, state, coins_view_cache, prev_block, Consensus::Params{}, tx_fee_out)) {
...
FUZZ_TARGET(coins_view_stacked, .init = initialize_coins_view) EXCLUSIVE_LOCKS_REQUIRED(!g_thread_pool_mutex)
{
    ...
    TestCoinsView(fuzzed_data_provider, backend_cache, backend_base_coins_view, /*is_db=*/true);
    CoinsViewOverlay coins_view_cache{&backend_cache, g_thread_pool, /*deterministic=*/true};
