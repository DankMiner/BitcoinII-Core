...
/**
 * Fee rate in satooshis per virtualbyte: CAmount / vB
 * the feerate is represented internally as FeeFrac
 */
class CFeeRate
{
private:
    /** Fee rate in sat2s/vB (satooshis per N virtualbytes) */
    FeePerVSize m_feerate;

public:
    /** Fee rate of 0 satooshis per 0 vB */
    CFeeRate() = default;
    ...
    std::string ToString(FeeRateFormat fee_rate_format = FeeRateFormat::BC2_KVB) const;
