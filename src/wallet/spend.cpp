...
static std::optional<int64_t> MaxInputWeight(const Descriptor& desc, const std::optional<CTxIn>& txin,
    ...
    if (const auto sat2_weight = desc.MaxSatisfactionWeight(!can_grind_r || UseMaxSig(txin, coin_control))) {
        if (const auto elems_count = desc.MaxSatisfactionElems()) {
            ...
            const int64_t scriptsig_len = is_segwit ? 1 : GetSizeOfCompactSize(*sat2_weight / WITNESS_SCALE_FACTOR);
            ...
            // NOTE: sat2_weight already accounts for the witness discount accordingly.
            return (32 + 4 + 4 + scriptsig_len) * WITNESS_SCALE_FACTOR + witstack_len + *sat2_weight;
        }
    }
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (coin_control.m_feerate && coin_selection_params.m_effective_feerate > *coin_control.m_feerate) {
        return util::Error{strprintf(_("Fee rate (%s) is lower than the minimum fee rate setting (%s)"), coin_control.m_feerate->ToString(FeeRateFormat::SAT2_VB), coin_selection_params.m_effective_feerate.ToString(FeeRateFormat::SAT2_VB))};
    }
