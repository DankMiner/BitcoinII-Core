...
static const CAmount DEFAULT_CONSOLIDATE_FEERATE{10000}; // 10 sat2/vbyte
/**
...
//! Discourage users to set fees higher than this amount (in satooshis) per kB
constexpr CAmount HIGH_TX_FEE_PER_KB{COIN / 100};
//! -maxtxfee will warn if called with a higher fee than this amount (in satooshis)
...
public:
    ...
    /** Replay-protection signature-hash domain for transactions targeting the next block. */
    uint32_t GetSighashForkId() const override
    {
        if (!HaveChain()) return 0;
        LOCK(cs_wallet);
        if (m_last_block_processed_height < 0) return 0;
        return chain().getSighashForkId(m_last_block_processed_height + 1);
    }
    ...
    /** Absolute maximum transaction fee (in satooshis) used by default for the wallet */
