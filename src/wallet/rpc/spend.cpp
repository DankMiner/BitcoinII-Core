...
static UniValue FinishTransaction(const std::shared_ptr<CWallet> pwallet, const UniValue& options, CMutableTransaction& rawTx)
{
    ...
    complete = FinalizeAndExtractPSBT(psbtx, mtx, pwallet->GetSighashForkId());
...
static RPCHelpMan bumpfee_helper(std::string method_name)
{
    ...
    const std::string incremental_fee{CFeeRate(DEFAULT_INCREMENTAL_RELAY_FEE).ToString(FeeRateFormat::SAT2_VB)};
    ...
    return RPCHelpMan{"sendall",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (coin_control.m_feerate && fee_rate > *coin_control.m_feerate) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Fee rate (%s) is lower than the minimum fee rate setting (%s)", coin_control.m_feerate->ToString(FeeRateFormat::SAT2_VB), fee_rate.ToString(FeeRateFormat::SAT2_VB)));
            }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (complete) {
        ...
        CHECK_NONFATAL(FinalizeAndExtractPSBT(psbtx, mtx, wallet.GetSighashForkId()));
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "changepos", "The position of the added change output, or -1"},
                                ...
                                RPCExamples{
                            "\nCreate a PSBT with automatically picked inputs that sends 0.5 BC2 to an address and has a fee rate of 2 sat2/vB:\n"
