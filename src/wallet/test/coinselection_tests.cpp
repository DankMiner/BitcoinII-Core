...
static CoinSelectionParams init_default_params()
{
    ...
    dcsp.m_change_fee = /*155 sat2s=*/dcsp.m_effective_feerate.GetFee(dcsp.change_output_size);
    dcsp.min_viable_change = /*204 sat2s=*/dcsp.m_discard_feerate.GetFee(dcsp.change_spend_size);
    dcsp.m_cost_of_change = /*204 + 155 sat2s=*/dcsp.min_viable_change + dcsp.m_change_fee;
...
BOOST_AUTO_TEST_CASE(bnb_test)
{
    ...
    for (int feerate : feerates) {
        ...
        // BnB fails to find changeless solution when overshooting by cost_of_change + 1 sat2
         ...
         * is 8 CENT while all UTXOs are slightly bigger than 1 CENT. The smallest eight are 100,000…100,007 sat2s, while
         * the larger nine are 100,368…100,375 (i.e., 100,008…100,016 sat2s plus cost_of_change (359 sat2s)).
         *
...
BOOST_AUTO_TEST_CASE(bnb_feerate_sensitivity_test)
{
    ...
    std::vector<OutputGroup> low_feerate_pool; // 5 sat2/vB (default, and lower than long_term_feerate of 10 sat2/vB)
    ...
    std::vector<OutputGroup> high_feerate_pool; // 25 sat2/vB (greater than long_term_feerate of 10 sat2/vB)
