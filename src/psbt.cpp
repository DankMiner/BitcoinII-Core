...
PrecomputedTransactionData PrecomputePSBTData(const PartiallySignedTransaction& psbt, uint32_t sighash_fork_id)
{
...
bool FinalizePSBT(PartiallySignedTransaction& psbtx, uint32_t sighash_fork_id)
{
    ...
    //   script.
    bool complete = true;
    const PrecomputedTransactionData txdata = PrecomputePSBTData(psbtx, sighash_fork_id);
...
bool FinalizeAndExtractPSBT(PartiallySignedTransaction& psbtx, CMutableTransaction& result, uint32_t sighash_fork_id)
{
    // It's not safe to extract a PSBT that isn't finalized, and there's no easy way to check
    //   whether a PSBT is finalized without finalizing it, so we just do this.
    if (!FinalizePSBT(psbtx, sighash_fork_id)) {
        return false;
    }
