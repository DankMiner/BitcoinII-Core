...
MutableTransactionSignatureCreator::MutableTransactionSignatureCreator(const CMutableTransaction& tx, unsigned int input_idx, const CAmount& amount, int hash_type, uint32_t sighash_fork_id)
    : m_txto{tx}, nIn{input_idx}, nHashType{hash_type}, amount{amount}, m_sighash_fork_id{sighash_fork_id}, checker{&m_txto, nIn, amount, MissingDataBehavior::FAIL, sighash_fork_id},
      m_txdata(nullptr)
{
}

MutableTransactionSignatureCreator::MutableTransactionSignatureCreator(const CMutableTransaction& tx, unsigned int input_idx, const CAmount& amount, const PrecomputedTransactionData* txdata, int hash_type, uint32_t sighash_fork_id)
    : m_txto{tx}, nIn{input_idx}, nHashType{hash_type}, amount{amount},
      m_sighash_fork_id{sighash_fork_id != 0 ? sighash_fork_id : (txdata ? txdata->m_sighash_fork_id : 0)},
      checker{txdata ? MutableTransactionSignatureChecker{&m_txto, nIn, amount, *txdata, MissingDataBehavior::FAIL, m_sighash_fork_id} :
                       MutableTransactionSignatureChecker{&m_txto, nIn, amount, MissingDataBehavior::FAIL, m_sighash_fork_id}},
      m_txdata(txdata)
{
}
...
bool MutableTransactionSignatureCreator::CreateSig(const SigningProvider& provider, std::vector<unsigned char>& vchSig, const CKeyID& address, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    uint256 hash = SignatureHash(scriptCode, m_txto, nIn, hashtype, amount, sigversion, m_txdata, nullptr, m_sighash_fork_id);
...
std::optional<uint256> MutableTransactionSignatureCreator::ComputeSchnorrSignatureHash(const uint256* leaf_hash, SigVersion sigversion) const
{
    ...
    uint256 hash;
    if (!SignatureHashSchnorr(hash, execdata, m_txto, nIn, nHashType, sigversion, *m_txdata, MissingDataBehavior::FAIL, m_sighash_fork_id)) return std::nullopt;
    return hash;
}
...
struct Satisfier {
    ...
    miniscript::Availability Sat2SHA256(const std::vector<unsigned char>& hash, std::vector<unsigned char>& preimage) const {
        return MsLookupHelper(m_sig_data.sha256_preimages, hash, preimage);
    }
    miniscript::Availability Sat2RIPEMD160(const std::vector<unsigned char>& hash, std::vector<unsigned char>& preimage) const {
        return MsLookupHelper(m_sig_data.ripemd160_preimages, hash, preimage);
    }
    miniscript::Availability Sat2HASH256(const std::vector<unsigned char>& hash, std::vector<unsigned char>& preimage) const {
        return MsLookupHelper(m_sig_data.hash256_preimages, hash, preimage);
    }
    miniscript::Availability Sat2HASH160(const std::vector<unsigned char>& hash, std::vector<unsigned char>& preimage) const {
...
SignatureData DataFromTransaction(const CMutableTransaction& tx, unsigned int nIn, const CTxOut& txout, uint32_t sighash_fork_id)
{
    SignatureData data;
    ...
    // Get signatures
    MutableTransactionSignatureChecker tx_checker(&tx, nIn, txout.nValue, MissingDataBehavior::FAIL, sighash_fork_id);
...
bool SignTransaction(CMutableTransaction& mtx, const SigningProvider* keystore, const std::map<COutPoint, Coin>& coins, int nHashType, std::map<int, bilingual_str>& input_errors, uint32_t sighash_fork_id)
{
    ...
    for (unsigned int i = 0; i < mtx.vin.size(); ++i) {
        ...
        SignatureData sigdata = DataFromTransaction(mtx, i, coin->second.out, sighash_fork_id);
        // Only sign SIGHASH_SINGLE if there's a corresponding output:
        if (!fHashSingle || (i < mtx.vout.size())) {
            ProduceSignature(*keystore, MutableTransactionSignatureCreator(mtx, i, amount, &txdata, nHashType, sighash_fork_id), prevPubKey, sigdata);
        }
        ...
        if (!sigdata.complete && !VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i, amount, txdata, MissingDataBehavior::FAIL, sighash_fork_id), &serror)) {
