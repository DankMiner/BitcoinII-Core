...
struct PrecomputedTransactionData
{
    ...
    /**
     * Optional replay-protection signature domain. Zero means the historical
     * BitcoinII sighash exactly. This value is not transaction data and is not
     * part of the BIP143/BIP341 precomputation; validation sets it from the
     * block height and signing sets it from the chain state.
     */
    uint32_t m_sighash_fork_id{0};
...
template <class T>
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int32_t nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache = nullptr, SigHashCache* sighash_cache = nullptr, uint32_t sighash_fork_id = 0);
...
template<typename T>
bool SignatureHashSchnorr(uint256& hash_out, ScriptExecutionData& execdata, const T& tx_to, uint32_t in_pos, uint8_t hash_type, SigVersion sigversion, const PrecomputedTransactionData& cache, MissingDataBehavior mdb, uint32_t sighash_fork_id = 0);
...
private:
    ...
    const uint32_t m_sighash_fork_id;
...
public:
    GenericTransactionSignatureChecker(const T* txToIn, unsigned int nInIn, const CAmount& amountIn, MissingDataBehavior mdb, uint32_t sighash_fork_id = 0) : txTo(txToIn), m_mdb(mdb), nIn(nInIn), amount(amountIn), txdata(nullptr), m_sighash_fork_id(sighash_fork_id) {}
    GenericTransactionSignatureChecker(const T* txToIn, unsigned int nInIn, const CAmount& amountIn, const PrecomputedTransactionData& txdataIn, MissingDataBehavior mdb, uint32_t sighash_fork_id = 0) : txTo(txToIn), m_mdb(mdb), nIn(nInIn), amount(amountIn), txdata(&txdataIn), m_sighash_fork_id(sighash_fork_id != 0 ? sighash_fork_id : txdataIn.m_sighash_fork_id) {}
