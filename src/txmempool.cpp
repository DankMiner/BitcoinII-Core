...
void CTxMemPool::check(const CCoinsViewCache& active_coins_tip, const CBlockIndex& prev_block, const Consensus::Params& params) const
{
    ...
    for (const auto& it : score_with_topo) {
        ...
        assert(Consensus::CheckTxInputs(tx, dummy_state, mempoolDuplicate, prev_block, params, txfee));
