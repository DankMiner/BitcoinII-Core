...
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_bitcoinII [ARGS]'.
  chainstate [ARGS] Run bitcoinII kernel chainstate util, equivalent to running 'bitcoinII-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_bitcoinII [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_bitcoinII-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "bitcoinII-gui" : "bitcoinII-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "bitcoinII-node" : "bitcoinII-d");
        } else if (cmd.command == "rpc") {
            args.emplace_back("bitcoinII-cli");
            // Since "bitcoinII rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("bitcoinII-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("bitcoinII-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_bitcoinII");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("bitcoinII-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_bitcoinII");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_bitcoinII-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("bitcoinII-util");
        } else {
...
//!
//!     strace -e trace=execve -s 10000 build/bin/bitcoinII ...
