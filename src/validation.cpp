    ...
    const Wtxid& wtxid)
{
    ...
    if (window_start == 0 ||
        ...
        if (suppressed > 0) {
            LogInfo(
                "BitcoinII transaction rules suppressed %u "
                "duplicate or rate-limited rejection log entries "
...
bool CheckBitcoinIIConsensusOutputs(
    const CTransaction& tx,
    BlockValidationState& state)
{
    switch (Consensus::CheckBitcoinIIOutputRules(tx)) {
    case Consensus::BitcoinIIOutputViolation::NONE:
        return true;

    case Consensus::BitcoinIIOutputViolation::OP_RETURN_COUNT:
        return state.Invalid(
            BlockValidationResult::BLOCK_CONSENSUS,
            "bad-txns-opreturn-count",
            "BitcoinII permits at most one OP_RETURN output per transaction");

    case Consensus::BitcoinIIOutputViolation::OP_RETURN_SIZE:
        return state.Invalid(
            BlockValidationResult::BLOCK_CONSENSUS,
            "bad-txns-opreturn-size",
            "BitcoinII OP_RETURN allowance exceeded");

    case Consensus::BitcoinIIOutputViolation::OP_RETURN_OP13:
        return state.Invalid(
            BlockValidationResult::BLOCK_CONSENSUS,
            "bad-txns-opreturn-op13",
            "BitcoinII forbids OP_13 in OP_RETURN");

    case Consensus::BitcoinIIOutputViolation::BARE_MULTISIG:
        return state.Invalid(
            BlockValidationResult::BLOCK_CONSENSUS,
            "bad-txns-bare-multisig",
            "BitcoinII forbids bare multisig outputs");
    }

    return false;
}

bool CheckBitcoinIIConsensusWitness(
    const CTransaction& tx,
    ...
    BlockValidationState& state)
{
    for (size_t i = 0; i < tx.vin.size(); ++i) {
        ...
        switch (Consensus::CheckBitcoinIIWitnessRules(
            coin.out.scriptPubKey,
            tx.vin[i].scriptWitness)) {
        case Consensus::BitcoinIIWitnessViolation::NONE:
            break;

        case Consensus::BitcoinIIWitnessViolation::TAPROOT_ANNEX:
            return state.Invalid(
                BlockValidationResult::BLOCK_CONSENSUS,
                "bad-txns-taproot-annex",
                "BitcoinII forbids Taproot annex data");

        case Consensus::BitcoinIIWitnessViolation::TAPSCRIPT_SIZE:
            return state.Invalid(
                BlockValidationResult::BLOCK_CONSENSUS,
                "bad-txns-tapscript-size",
                "BitcoinII Taproot script-path size limit exceeded");

        case Consensus::BitcoinIIWitnessViolation::ORDINAL_ENVELOPE:
            return state.Invalid(
                BlockValidationResult::BLOCK_CONSENSUS,
                "bad-txns-ordinal-envelope",
                "BitcoinII forbids Ordinals inscription envelopes");
        }
    }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    auto log_policy_rejection = [&](
        ...
        if (!ShouldLogPolicyRejection(wtxid)) {
            return;
        }
        ...
        LogInfo(
            "BitcoinII transaction rules refused to relay transaction "
    ...
    case Consensus::BitcoinIIOutputViolation::OP_RETURN_SIZE:
        log_policy_rejection(
            "OP_RETURN/data-carrier",
            "bitcoinII-opreturn-size",
            std::nullopt,
            GetDataCarrierBytes(tx),
            MAX_BITCOINII_OP_RETURN_BYTES);
        return state.Invalid(
            TxValidationResult::TX_NOT_STANDARD,
            "bitcoinII-opreturn-size");
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // Mempool signatures must be valid for the next block, including a replay
    // protection activation that may differ from the current tip's domain.
    ws.m_precomputed_txdata.m_sighash_fork_id =
        args.m_chainparams.GetConsensus().SighashForkId(
                       ...
                       std::vector<CScriptCheck>* pvChecks)
{
    ...
    hasher.Write(UCharCast(tx.GetWitnessHash().begin()), 32)
        ...
        .Write((unsigned char*)&txdata.m_sighash_fork_id, sizeof(txdata.m_sighash_fork_id))
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    const bool enforce_bitcoinII_data_rules{
        pindex->nHeight >=
        params.GetConsensus().nDataRestrictionsHeight
    };
    ...
    const uint32_t sighash_fork_id =
        params.GetConsensus().SighashForkId(pindex->nHeight);
    for (auto& txdata : txsdata) {
        txdata.m_sighash_fork_id = sighash_fork_id;
    }
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (enforce_bitcoinII_data_rules &&
            !CheckBitcoinIIConsensusOutputs(tx, state)) {
            break;
        }
    ...
    DisconnectedBlockTransactions& disconnectpool)
{
    ...
    /*
     * The block immediately before replay-protection activation is the last
     * point at which the mempool can contain transactions signed in the legacy
     * domain. Those signatures are intentionally invalid in the next block.
     * Flush the mempool once at this boundary so block assembly cannot inherit
     * stale pre-fork signatures. New post-fork-domain transactions may be
     * accepted immediately afterward for the activation block.
     */
    const Consensus::Params& consensus = m_chainman.GetConsensus();
    if (m_mempool &&
        consensus.nReplayProtectionForkId != 0 &&
        pindexNew->nHeight + 1 == consensus.nReplayProtectionHeight) {
        const auto remove_legacy_domain = [](CTxMemPool::txiter) { return true; };
        m_mempool->removeForReorg(m_chain, remove_legacy_domain);
        LogInfo("Cleared mempool before replay-protection activation at block #%d.",
                consensus.nReplayProtectionHeight);
    }
...
double ChainstateManager::GuessVerificationProgress(const CBlockIndex* pindex) const
{
    ...
    const auto block_time{
        (Assume(m_best_header) && std::abs(nNow - pindex->GetBlockTime()) <= Ticks<std::chrono::seconds>(12h) &&
