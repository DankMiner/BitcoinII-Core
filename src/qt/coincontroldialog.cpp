...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of satooshis so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    // how many satooshis the estimated fee can vary per byte we guess wrong
    double dFeeVary = (nBytes != 0) ? (double)nPayFee / nBytes : 0;

    QString toolTip4 = tr("Can vary +/- %1 satooshi(s) per input.").arg(dFeeVary);
