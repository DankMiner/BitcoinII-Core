...
//! Specify whether parameter should be parsed by bitcoinII-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make bitcoinII-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `bitcoinII-cli` attempt to parse the
 ...
 * `bitcoinII-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make bitcoinII-cli treat them as positional parameters when `-named` is used.
 * This prevents `bitcoinII-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so bitcoinII-cli
 ...
 *
 * For example, the command `bitcoinII-cli -named createwallet "my=wallet"`,
